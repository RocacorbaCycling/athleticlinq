          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone/30 p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                {athlete.procyclingstatsUrl && (
                  <a
                    href={athlete.procyclingstatsUrl}
                    target="_blank"
              ...
              {athlete.pcsResults?.length ? (
                <ul className="space-y-2.5">
                  {athlete.pcsResults.map((r, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm">
                      ...
                      <a href={r.raceUrl} target="_blank" rel="noopener noreferrer"
                        className="flex-1 text-warm-black hover:text-coral transition-colors">
                        {r.race}
                        {r.year && <span className="text-earth/50 ml-1 text-xs">{r.year}</span>}
                      </a>
                      {r.category && (
                        <span className="text-[10px] font-medium text-earth/50 bg-stone/30 px-1.5 py-0.5 rounded-full shrink-0">
                          {r.category}
                        </span>
                      )}
                    </li>
                ...
                /* Fall back to manually entered palmares */
                <ul className="space-y-3">
                  {athlete.palmares.map((result, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold
                        ${result.startsWith("1st") ? "bg-coral/10 text-coral" : result.startsWith("2nd") ? "bg-navy/10 text-navy" : "bg-stone text-earth"}`}>
